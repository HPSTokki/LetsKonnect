...
dateDisplay.addEventListener("click", () => {
  dateTimePicker.style.display = dateTimePicker.style.display === "block" ? "none" : "block";
});
...
saveDate.addEventListener("click", () => {
  ...
  if (date && time) {
    ...
    const formattedDateString = `${(formattedDate.getMonth() + 1).toString().padStart(2, '0')}/${formattedDate.getDate().toString().padStart(2, '0')}/${formattedDate.getFullYear().toString().slice(-2)}`;
